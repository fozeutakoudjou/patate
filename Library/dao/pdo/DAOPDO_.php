...
class DAOPDO extends DAO{
    ...
    protected static $operatorList = array(
		...
		Operator::CONTAINS => '%s LIKE "%%%s%%"',
		Operator::START_WITH => '%s LIKE "%s%%"',
		Operator::END_WITH => '%s LIKE "%%%s"',
	);
     ...
     * @return bool
     */
    protected function _update($model, $fieldsToUpdate = array(), $identifiers = array()) {
        $fieldsString ='';
        $first = true;
	...
	protected function _getByFieldsCount($fields, $logicalOperator = LogicalOperator::AND_){
		$restriction=$this->getRestrictionFromArray($fields, $logicalOperator);
    ...
    protected function addModelParam($query, $model, $field, $suffix = '_tmp', $lang = false) {
        $keyTmp = $field.$suffix;
    ...
    protected function getOperatorQuery($field, $value, $operator) {
		...
		if(isset(self::$operatorList[$operator])){
			$sql .= sprintf(self::$operatorList[$operator], $field, ':'.$field);
			...
			foreach ($this->languages as $lang => $langObject){
				...
				foreach ($langFields as $field){
					if (property_exists($model, $field)){
						$req->bindParam(':'.$field, $model->getPropertyValue($field)[$lang]);
					} 
				}
