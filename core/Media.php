...
class MediaCore
{
    ...
    public static function packJSinHTML($html_content)
    {
        if (strlen($html_content) > 0) {
            ...
            if (!preg_match('/'.self::$pattern_keepinline.'/', $html_content)) {
                    $html_content = preg_replace_callback(
                    self::$pattern_js,
    ...
    {
        self::$current_css_file = $fileuri;

        if (strlen($css_content) > 0) {
            $limit  = self::getBackTrackLimit();
            $css_content = preg_replace('#/\*.*?\*/#s', '', $css_content, $limit);
            $css_content = preg_replace_callback(self::$pattern_callback, array('Media', 'replaceByAbsoluteURL'), $css_content, $limit);
            $css_content = preg_replace('#\s+#', ' ', $css_content, $limit);
    ...
    public static function replaceByAbsoluteURL($matches)
    {
        if (array_key_exists(1, $matches) && array_key_exists(2, $matches)) {
            if (!preg_match('/^(?:https?:)?\/\//iUs', $matches[2])) {
                ...
                $sep = '/';
                $tmp = $matches[2][0] == $sep ? $matches[2] : dirname(self::$current_css_file).$sep.ltrim($matches[2], $sep);
    ...
    public static function getJqueryPath($version = null, $folder = null, $minifier = true)
    {
        ...
        if (@filemtime($file_uri) || (defined('_PS_HOST_MODE_') && @filemtime($file_uri_host_mode))) {
            ...
            $return[] = self::getJSPath(Tools::getCurrentUrlProtocolPrefix().'ajax.googleapis.com/ajax/libs/jquery/'
                .$version.'/jquery'.($minifier ? '.min.js' : '.js'));
        }

        if ($add_no_conflict) {
            $return[] = self::getJSPath(Context::getContext()->shop->getBaseURL(true, false)._PS_JS_DIR_
        ...
        $return[] = self::getJSPath(_PS_JS_DIR_.'jquery/jquery-migrate-1.2.1.min.js');
    ...
    public static function getJqueryUIPath($component, $theme, $check_dependencies)
    {
        ...
        $file_uri_host_mode = _PS_CORE_DIR_.Tools::str_replace_once(__PS_BASE_URI__, DIRECTORY_SEPARATOR, $url_data['path']);
        $ui_tmp = array();
        if (isset(self::$jquery_ui_dependencies[$component]) && self::$jquery_ui_dependencies[$component]['theme'] && $check_dependencies) {
        ...
        if ($check_dependencies && array_key_exists($component, self::$jquery_ui_dependencies)) {
            foreach (self::$jquery_ui_dependencies[$component]['dependencies'] as $dependency) {
                ...
                if (self::$jquery_ui_dependencies[$dependency]['theme']) {
                    $dep_css = self::getCSSPath($folder.'themes/'.$theme.'/jquery.'.$dependency.'.css');
                }
    ...
    public static function getJqueryPluginPath($name, $folder = null)
    {
        ...
        $plugin_path['css'] = self::getJqueryPluginCSSPath($name, $folder);
            ...
            foreach ($js_files_infos as $file_infos) {
                if (file_exists($file_infos['path'])) {
                    ...
                    if (preg_match('@\.(min|pack)\.[^/]+$@', $file_infos['path'], $matches)) {
                        ...
                        $content .= self::packJS($tmp_content);
                    }
                } else {
                    ...
                    foreach ($patterns as $pattern) {
                        ...
                        if (preg_match($pattern, $src, $matches)) {
                            ...
                            if ($version) {
                                ...
                                array_push(self::$inline_script_src, $src);
                            }
                        }
                    }
                    if (!in_array($src, self::$inline_script_src) && !$script->getAttribute(self::$pattern_keepinline)) {
                        Context::getContext()->controller->addJS($src);
                    }
                }
            }
        }
        $output = preg_replace_callback(self::$pattern_js, array('Media', 'deferScript'), $output);
        return $output;
    }
    ...
    public static function deferScript($matches)
    {
        ...
        if (!empty($inline) && preg_match(self::$pattern_js, $original) !== false && !preg_match('/'.self::$pattern_keepinline.'/', $original) && self::$inline_script[] = $inline) {
            return '';
        }
        ...
        if (array_key_exists(1, $results)) {
            ...
            if (in_array($results[1], Context::getContext()->controller->js_files) || in_array($results[1], self::$inline_script_src)) {
                return '';
            }
        }
